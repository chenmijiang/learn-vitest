...
- [[Testing Methodology|topics/testing-methodology.md]] - 工程化测试方法论：L0 价值（变更信心/北极星=逃逸 bug/ROI 取舍）→L1 策略（公共 API 边界、前端奖杯·后端金字塔、风险驱动）→L2 战术（IO 边界 mock、等价类+边界值、覆盖率防倒退+变异测试）→L3 运营（分层反馈、`--changed`/`--shard`/`pool`），以及 AI 辅助层（实现=上下文·规格=断言源、自动护栏为主人只审意图）。`updated: 2026-06-07` `sources: 6`
