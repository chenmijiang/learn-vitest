...
- 派发任务给 subagent 时，使用的模型范围仅考虑 `gpt-5.4-mini`、`gpt-5.4`、`kimi-for-coding`
- 触发并执行 Wiki Maintainer Skill 后，**必须**commit wiki改动，并在 message 注明改动类型和相关信息，eg: `docs(wiki): backfill topic page for vitest configuration`，确保 wiki 的稳定性和可追溯性。

## References

- [Wiki Orchestration Rules](./.agents/rules/wiki-orchestration-rules.md)
- [Wiki Maintainer Skill](./.agents/skills/wiki-maintainer/SKILL.md): Maintain the wiki as a stable knowledge layer between raw docs and user answers. Prefer updating existing topic pages over creating new pages.
- [Conversation Summary Skill](./.agents/skills/conversation-summary/SKILL.md): 一个规范对话内容到项目文档的组织流程，可以确保文档清晰、准确、易懂。
